...
**Claude** ([claude.ai/download](https://claude.ai/download) o el CLI `claude`) puede leer y escribir archivos en tu computadora. Cuando instalas Mind++, Claude aprende una forma específica de organizar tus notas dentro de esa carpeta — así, cuando dices "tuve una reunión con Juan", escribe el acta, extrae las acciones y actualiza tu lista de pendientes automáticamente.
...
|-------------|----------|------|
| **Claude** (obligatorio) | App de escritorio o CLI — corre el plugin | [claude.ai/download](https://claude.ai/download) · [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code/overview) |
...
Elige el camino que corresponde a cómo usas Claude:

**Opción A — App de escritorio de Claude** ([claude.ai/download](https://claude.ai/download))
...
2. Abre Claude → **Configuración** → **Plugins**
3. Arrastra el archivo `.plugin` al panel de plugins (o haz clic en **Instalar desde archivo**)

**Opción B — Claude Code (CLI)**

1. Agrega el marketplace de Mind++ una vez:
   ```bash
...
2. Instala el plugin:
   ...
   ```
3. Reinicia Claude Code (`/quit`, luego vuelve a abrir).

### 4. Apunta Claude a la carpeta de tu vault

**App de escritorio:** En Claude → **Configuración**, selecciona la carpeta que elegiste en el paso 2 como tu workspace. Claude leerá y escribirá directamente en ella.

**Claude Code (CLI):** Abre una sesión de terminal dentro de la carpeta de tu vault, o inicia Claude Code desde ese directorio. Claude Code usa el directorio de trabajo actual como raíz del proyecto.
...
Claude puede correr cualquier skill en un schedule. El setup ofrece crear:
...
Todo se queda en tu computadora. Mind++ no envía tus datos a ningún lado. Los únicos servicios externos involucrados son los que conectes explícitamente en Claude (por ejemplo, Gmail para el pipeline de reuniones, Google Calendar para el daily brief) — y son opt-in.
...
Construido con [Claude](https://claude.ai) de Anthropic. Potenciado por [Claude](https://claude.ai/download) e inspirado por la comunidad [Obsidian](https://obsidian.md).
